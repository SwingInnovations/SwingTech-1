...
void GLMesh::load(std::ifstream &in, bool hasBones) {
    ...
    stInt numVert = mesh.getVertexSize();
    m_drawCount = (stUint)mesh.getIndexSize();

    for(stInt i = 0; i < numVert; i++){
        vertex.push_back(*mesh.m_vertices[i].getVertex());
        texCoord.push_back(*mesh.m_vertices[i].getTexCoord());
        normal.push_back(*mesh.m_vertices[i].getNormal());
    }
    index = mesh.m_indices;
    tangent = genTangent(vertex, texCoord);
    biTangent = genBiTangent(vertex, texCoord);
