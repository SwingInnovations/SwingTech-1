...
OBJMesh::OBJMesh(const std::string &filename) {
    ...
    while(std::getline(in, line)){
        ...
        if(line[0] == 'f' && line[1] == ' '){
            ...
            std::string face1 = hLine.substr(0, hLine.find(' '));
                ...
                std::string t1 = face1.substr(face1.find('/'), midLen1);
                j = (int)atoi(t1.c_str());
            ...
            std::string face2 = hLine.substr(hLine.find(' '), midLen);
                std::string v2 = face2.substr(0, face2.find(' '));
                i = (int) atoi(v2.c_str());
                int midLen2 = face2.find_last_of('/')-face2.find('/')+1;
                std::string t2 = face2.substr(face2.find('/'), midLen2);
                j = (int)atoi(t2.c_str());
            ...
            std::string face3 = hLine.substr(hLine.find_last_of('/') + 1);
                ...
                std::string t3 = face3.substr(face3.find('/'), midLen3);
                j = (int)atoi(t3.c_str());
    ...
    for(unsigned int i = 0, S = _index.size(); i < S; i++){
        verticies.push_back(Vertex(_vertex.at(_index.at(i)), _texCoord.at(_index.at(i)), _normal.at(_index.at(i))));
